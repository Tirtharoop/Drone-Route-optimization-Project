...
def apply_dtrc(truck_routes, distances_df, demands, d_capacity):
    ...
    for route_idx, truck_route in enumerate(truck_routes):
        ...
        i = 0  # Start from depot (can now launch from any position including segments)
        while i < len(truck_route) - 1:  # Until the last node before returning to depot
            ...
            for drone_idx in range(2):
                ...
                if potential_deliveries:
                    # MODIFICATION: Only use actual delivery points for takeoff (no virtual takeoff points)
                    # Use the current node as the takeoff point (which is always a delivery point or depot)
                    takeoff_point = current_node
                    is_virtual_takeoff = False
                    ...
                    for num_nodes in range(1, max_nodes + 1):
                        # Try various combinations of nodes (maintaining order)
                        ...
                        for combo_indices in combinations(range(min(6, len(potential_deliveries))), min(num_nodes, len(potential_deliveries[:6]))):
                            ...
                            # Delivery sequence must follow truck route order (already sorted)
                            delivery_sequence = delivery_nodes
                            
                            # Generate potential rendezvous points for landing on the same truck
                            rendezvous_points = []
                            
                            # Find last delivery node's position in truck route
                            last_delivery_idx = max(indices)
                            
                            # Include actual nodes as rendezvous points (only on the same truck)
                            ...
                            # Add intermediary points between nodes but ONLY on truck route segments
                            for segment_idx in range(last_delivery_idx + 1, len(truck_route) - 1):
                                ...
                                # Create virtual rendezvous points along this segment of the truck route
                            ...
                            # For each possible rendezvous point
                            for landing_point, landing_idx, is_virtual in rendezvous_points:
...
drone_routes, modified_truck_routes = apply_dtrc(truck_routes, distances_df, demands, d_capacity)
