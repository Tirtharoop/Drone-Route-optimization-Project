...
def apply_dtrc(truck_routes, distances_df, demands, d_capacity, x_coords, y_coords):
    ...
    def is_point_covered_by_truck(takeoff_idx, i):
        # Only allow takeoff if it's at or after the current truck node, but before the next node
        return i <= takeoff_idx < i + 1
        ...
        while i < len(truck_route) - 1:
            ...
            for drone_idx in range(2):
                ...
                if potential_deliveries:
                    takeoff_points = []
                    # Always allow current node as a takeoff point
                    takeoff_points.append((current_node, i, False))
                    # Virtual points along the segment
                    if i < len(truck_route) - 1:
                        for step in [0.25, 0.5, 0.75]:
                            virtual_x = (1-step) * x_coords[current_node] + step * x_coords[next_node]
                            virtual_y = (1-step) * y_coords[current_node] + step * y_coords[next_node]
                            virtual_takeoff = f"V({virtual_x:.1f},{virtual_y:.1f})"
                            takeoff_points.append((virtual_takeoff, i + step, True))

                    # Filter: keep only takeoff points covered by truck at this time
                    valid_takeoff_points = []
                    for takeoff_point, takeoff_idx, is_virtual in takeoff_points:
                        if is_point_covered_by_truck(takeoff_idx, i):
                            valid_takeoff_points.append((takeoff_point, takeoff_idx, is_virtual))
                    # If none, adjust to nearest valid point (fallback: use current node)
                    if not valid_takeoff_points:
                        valid_takeoff_points = [(current_node, i, False)]

                    best_route = None
                    best_value = 0
                    best_landing_idx = None

                    for takeoff_point, takeoff_idx, is_virtual_takeoff in valid_takeoff_points:
                        max_nodes = min(3, len(potential_deliveries))
                        for num_nodes in range(1, max_nodes + 1):
                            for combo_indices in combinations(range(min(6, len(potential_deliveries))), min(num_nodes, len(potential_deliveries[:6]))):
                                ...
                                if total_payload > DRONE_CAPACITY:
                                    continue
                                delivery_sequence = delivery_nodes
                                rendezvous_points = []
                                last_delivery_idx = max(indices)
                                for landing_idx in range(last_delivery_idx + 1, len(truck_route)):
                                    landing_node = truck_route[landing_idx]
                                    rendezvous_points.append((landing_node, landing_idx, False))
                                for segment_idx in range(last_delivery_idx + 1, len(truck_route) - 1):
                                    node1 = truck_route[segment_idx]
                                    node2 = truck_route[segment_idx + 1]
                                    for step in [0.25, 0.5, 0.75]:
                                        virtual_x = (1-step) * x_coords[node1] + step * x_coords[node2]
                                        virtual_y = (1-step) * y_coords[node1] + step * y_coords[node2]
                                        virtual_landing = f"V({virtual_x:.1f},{virtual_y:.1f})"
                                        rendezvous_points.append((virtual_landing, segment_idx + step, True))
                                for landing_point, landing_idx, is_virtual_landing in rendezvous_points:
                                    if landing_idx <= takeoff_idx:
                                        continue
                                    route = [takeoff_point]
                                    route.extend(delivery_sequence)
                                    ...
                                    for idx2 in range(len(route) - 1):
                                        from_node = route[idx2]
                                        to_node = route[idx2 + 1]
                                    ...
                                    if valid_route:
                                        value = len(delivery_sequence) / (total_dist + 0.1)
                                        if value > best_value:
                                            best_route = route
                                            best_value = value
                    ...
                    if best_route:
                        ...
                        for node in best_route[1:-1]:
                            if not isinstance(node, str):
                                used_nodes.add(node)
                        drone_positions[drone_idx] = best_landing_idx
