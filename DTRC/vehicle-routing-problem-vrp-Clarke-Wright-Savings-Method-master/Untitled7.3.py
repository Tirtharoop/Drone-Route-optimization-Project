...
def apply_dtrc(truck_routes, distances_df, demands, d_capacity, x_coords, y_coords):
    ...
    for route_idx, route in enumerate(modified_truck_routes):
        ...
        all_drone_routes = validate_drone_virtual_points(all_drone_routes, modified_truck_routes, x_coords, y_coords)
    
    return all_drone_routes, modified_truck_routes

def validate_drone_virtual_points(all_drone_routes, modified_truck_routes, x_coords, y_coords):
    """
    Validates all virtual takeoff points to ensure they're on the current modified truck route.
    If not, shifts the takeoff point to the nearest point on the truck route.
    """
    import math
    
    # For each truck's drone routes
    for truck_idx, truck_drone_routes in enumerate(all_drone_routes):
        # Skip if truck has no route
        if truck_idx >= len(modified_truck_routes) or not modified_truck_routes[truck_idx]:
            continue
            
        truck_route = modified_truck_routes[truck_idx]
        
        # Create line segments for this truck's route
        truck_segments = []
        for i in range(len(truck_route) - 1):
            ...
            truck_segments.append((
                start_node, end_node,
                (x_coords[start_node], y_coords[start_node]),
                (x_coords[end_node], y_coords[end_node])
            ))
        
        # Process each drone's routes for this truck
        for drone_idx, drone_routes in enumerate(truck_drone_routes):
            for route_idx, route in enumerate(drone_routes):
                if not route:
                    continue
                
                # Check if takeoff is virtual
                takeoff = route[0]
                if isinstance(takeoff, str) and takeoff.startswith('V('):
                    ...
                    if len(coords) == 2:
                        try:
                            v_x, v_y = float(coords[0]), float(coords[1])
                            
                            # Check if this virtual point is on any segment of the modified truck route
                            on_route = False
                            best_segment = None
                            ...
                            for seg_idx, (start_node, end_node, start_coords, end_coords) in enumerate(truck_segments):
                                ...
                                seg_len_sq = (e_x - s_x)**2 + (e_y - s_y)**2
                                
                                # If segment is essentially a point, use distance to that point
                                if seg_len_sq < 0.0001:
                                    dist = math.sqrt((v_x - s_x)**2 + (v_y - s_y)**2)
                                else:
                                    # Project point onto line segment
                                    t = max(0, min(1, ((v_x - s_x) * (e_x - s_x) + 
                                                      (v_y - s_y) * (e_y - s_y)) / seg_len_sq))
                                    
                                    # Nearest point on segment
                                    proj_x = s_x + t * (e_x - s_x)
                                    proj_y = s_y + t * (e_y - s_y)
                                    
                                    # Distance to nearest point
                                    dist = math.sqrt((v_x - proj_x)**2 + (v_y - proj_y)**2)
                                
                                # Check if point is near enough to segment (tolerance of 0.1)
                                if dist < 0.1:
                                    on_route = True
                                    break
                                    
                                # Keep track of best segment
                                if dist < best_distance:
                                    best_distance = dist
                                    best_segment = (seg_idx, start_node, end_node, t)
                            
                            # If not on route, shift to nearest point on nearest segment
                            if not on_route and best_segment:
                                seg_idx, start_node, end_node, t = best_segment
                                
                                # Get coordinates of nearest point on segment
                                s_x, s_y = x_coords[start_node], y_coords[start_node]
                                e_x, e_y = x_coords[end_node], y_coords[end_node]
                                
                                new_x = s_x + t * (e_x - s_x)
                                new_y = s_y + t * (e_y - s_y)
                                
                                # Create new virtual point
                                new_takeoff = f"V({new_x:.1f},{new_y:.1f})"
                ...
                # Also check landing point
                landing = route[-1]
                if isinstance(landing, str) and landing.startswith('V('):
                    # Similar process for landing points
                    ...
                    if len(coords) == 2:
                        try:
                            ...
                            if not on_route and best_segment:
                                ...
                                # Create new virtual point
                                new_landing = f"V({new_x:.1f},{new_y:.1f})"
